...
# 5. Single group with all models
group_all = [
    "Pitchfork", "Saddle", "Hopf", "Transcritical",
    "Lorentz", "Van der Pol", "Hodgkin-Huxley",
    "FitzHugh–Nagumo", "Lotka Volterra"
]
...
colors_map = { lab: pastel9[i] for i, lab in enumerate(group_all) }

# 6. Plotting function (same “good bars” style)
def plot_group_box(group, filename, title):
    ...
    out_pdf = os.path.join(
        plots_dir,
        f"{script_name}_{filename}.pdf"
    )
    fig.savefig(out_pdf, dpi=300, format="pdf", bbox_inches="tight")
    plt.close(fig)

# 7. Generate and save the single combined plot
plot_group_box(
    group_all,
    "all_models_group",
    "All Models — Error vs Noise"
)
