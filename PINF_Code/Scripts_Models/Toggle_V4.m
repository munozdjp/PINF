...
title('Polinomial dynamic of beta parameter');
hold off
xMax = [1 1 1 1];
...
%     i=i+1
    ...
    y0=3.0;
    [t,x] = ode45(@(t,x) toggle_switch(t,x,c,alpha2,beta,gamma,xMax,reescaleTimeJin,maxBeta),tspan,[tspan(1),mu0,x0,y0],options);
%     [t,x] = ode45(@(t,x) toggle_switch(t,x,weightdx,alpha2,beta,gamma,xMax,reescaleTimeJin,maxBeta),tspan,[tspan(1),mu0,x0,y0],options);
     X = [X;x];
...
mufunc = @(x) ((1+(x(:,4)*xMax(4)).^beta).*(x(:,3)*xMax(3)))/maxBeta;
...
%     time.^3*weightdx(4);
F1=@(weightdx,time) reescaleTimeJin*((weightdx(2)*time+time.^2*weightdx(3)+...
...
%replace our assummption of c1 = 0 
weightdx(1) = 0 
%Data predicted from F1 evaluated in the polinomail fit
reproduced_data= F1(weightdx,tspan);

plotState_Beta_time_Pred(tspan,x,mu_observed,yzero,reproduced_data,n,3,'Toggle',10)
...
% c-weights = of original trajectory
% weightdx = predicted with weights. 
% weightdxNoise = prediction with the main trajectory
comparisonVector = [c(1:(length(c)-1));weightdx;weightdxNoise]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


plotState_Beta_time_Pred(tspan,xNoisy,mu_obsNoisy,yzero,reproduced_dataNoisy,n,4,'Toggle',50)
...
mu0=[yzero(1)]; %initial condition of Beta
    x0=0.05/xMax(3); %Iniitial contions for x and y [x0,x0,z0]        
    y0=3.0/xMax(4);
    [t,xreconstruct] = ode45(@(t,x) toggle_switch(t,x,C_predicted ,alpha2,beta,gamma,xMax,reescaleTimeJin,maxBeta),tspan,[tspan(1),mu0,x0,y0],options);
%     [t,xreconstruct] = ode45(@(t,x) toggle_switch(t,x,weightdx ,alpha2,beta,gamma,xMax,reescaleTimeJin,maxBeta),tspan,[tspan(1),mu0,x0,y0],options);
     X = [X;xreconstruct];
        % Plot of state variable vs Beta parameter
        figure(6)
        hold on
...
%%
% save('ToggleOri_Reco.mat', 'xNoisy', 'xreconstruct','tspan','noise_scale');
save(fullfile(dataDir, 'Toggle_Reconstruction.mat'), 'xNoisy', 'xreconstruct','tspan','noise_scale');
%% Noise analysis 
